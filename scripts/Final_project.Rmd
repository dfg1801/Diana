...
scaled_phylo_dist_full <- phylo_dist %>% 
  melt() %>% #turn matrix into a dataframe
  mutate(Var1 = as.character(Var1)) %>% # these were factors, turn them into characters bc I don't need the factor value and so I can work with it easier
  mutate(Var2 = as.character(Var2)) %>%
  filter(Var1 != Var2) %>%
  ...
  mutate(Var2 = str_to_lower(Var2)) %>%
  mutate(pair = paste(Var1, ",", Var2)) %>%
  mutate(scaled_p_dist = abs(standardize(value))) # scale and abs distances

#now put the two together ----------
scaled_sim_dist <- cleaned_sim_dist %>%
   mutate(scaled_dist = abs(standardize(value))) %>%
  left_join(scaled_phylo_dist_full, by = join_by(pair)) %>%
  select(pair, scaled_dist, scaled_p_dist)
```
...
```{r, brownian_dist}
ggplot(scaled_sim_dist, aes(x = scaled_p_dist, y = scaled_dist)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Pairwise distances",
