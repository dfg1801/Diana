...
``` {r}
# For AVONET data -----------------------------------
#vcv_test <- vcv(tree, model = "Brownian", corr = FALSE)
tree_trim <- tree %>%
...
bm_l <- qfoldnorm(0.95, mean = 0, sd = sqrt(cleaned_phylo_dist$phylo_dist * length_sd$s2)) # sd of the data might be overestimated if pulled frmo real data, try vcv to see if this changes anything
...
bm_w <- qfoldnorm(0.95, mean = 0, sd = sqrt(cleaned_phylo_dist$phylo_dist * width_sd$s2)) # sd of the data might be overestimated if pulled frmo real data, try vcv to see if this changes anything
...
bm_d <- qfoldnorm(0.95, mean = 0, sd = sqrt(cleaned_phylo_dist$phylo_dist * depth_sd$s2)) # sd of the data might be overestimated if pulled frmo real data, try vcv to see if this changes anything
bm_depth <- data.frame(phylo_dist = cleaned_phylo_dist$phylo_dist, 
                        morpho_dist = bm_d, 
                        pair = cleaned_phylo_dist$pair)

# general morpho (NOT DONE YET)
bm_val <- qfoldnorm(0.95, mean = 0, sd = sqrt(cleaned_phylo_dist$phylo_dist * sig^2)) # sd of the data might be overestimated if pulled frmo real data, try vcv to see if this changes anything
...
bm_W_2L <- qfoldnorm(0.95, mean = 0, sd = sqrt(phylo_dist_3d$phylo_dist * W_2L_sd$s2)) # sd of the data might be overestimated if pulled frmo real data, try vcv to see if this changes anything
